...
> **Open methodological caveat, added 2026-08-10, affects every number in this document dated
> before it:** teacher-label generation (`src/01_generate_teacher.py`) sampled at
> `temperature=0.1` with no fixed seed through iteration 14 — meaning the ground-truth training
> labels themselves were non-deterministic across runs, even for byte-identical source corpora.
> This was diagnosed by comparing two iterations (5 and 12) that used an identical corpus and
> parameters yet produced hybrid-eval field F1 scores 0.043 apart — a larger swing than most of
> the deliberate experimental changes made since. Fixed as of the commit noted in iteration 15
> (greedy decoding, `temperature=0.0`, no sampling). **Results from iterations 3–14 should be read
> as measurements made under an uncontrolled noise source**, not necessarily as clean evidence for
> or against the specific hypothesis each iteration was framed to test. See §7 below.
...
- **Post-iteration-14 review**: comparing the full iteration history side by side (rather than
  ...
  14. Fixed (greedy decoding) as of iteration 15. **This means iterations 3 through 14's specific
  numeric findings should be read with real caution** — some fraction of the variance attributed to
  corpus scale, corpus composition, or ontology fills may actually be teacher-sampling noise that
  this fix removes going forward. A controlled re-run under the fixed pipeline is the natural next
  step to establish which prior findings hold up.
